...
void ZenFS::GCWorker() {
  while (run_gc_worker_) {
    ...
    /*
    std::vector<uint64_t> spaceL2 = zbd_->GetUsedCapacityL2(); // modi
    uint64_t L2non_free = spaceL2[0]; //modi
    //uint64_t L2free = spaceL2[1]; //modi
    */
    uint64_t non_free = zbd_->GetUsedSpace() + zbd_->GetReclaimableSpace();// - L2non_free; //modi
    uint64_t free = zbd_->GetFreeSpace();// - L2free; // modi
    ...
    for (const auto& zone : snapshot.zones_) {
      if (zone.capacity == 0) {
        ...
        if (garbage_percent_approx > threshold &&
          ...
          len += sprintf(buf+len, "GC %ld \n", zone.start);
  
          std::ofstream f3("test2.txt", std::ios::app);
          f3 << buf;
          f3.close();
        }
      }
...
IOStatus ZenFS::DeleteFileNoLock(std::string fname, const IOOptions& options,
  ...
  IOStatus s;
    ...
    len_te += sprintf(buf_te + len_te, "file name: %s\n", fname.c_str());
...
IOStatus ZenFS::MigrateFileExtents(
  ...
  /*modi*/
  //for(auto mm : er_zfile) {
    //ROCKS_LOG_INFO(db_options_.info_log,
//		    "wr: %ld, lifetime: %d\m", mm.first, mm.second);
    //fprintf(stdout, "wr: %ld, lifetime: %d\n", mm.first, mm.second);
    //Info(logger_, "wr: %ld, lifetime: %d\n", mm.first, mm.second);
  //}
  //fprintf(stdout, "\n");


  //Info(logger_, "sst id: %s", zfile->GetLinkFiles()[0].c_str());
  //Info(logger_, "lifetime: %d", zfile->GetWriteLifeTimeHint());

  //fprintf(stdout, "sst id: %s\n", zfile->GetLinkFiles()[0].c_str());
  //fprintf(stdout, "lifetime: %d\n\n", zfile->GetWriteLifeTimeHint());
  ...
  /*
    ...
    len_m += sprintf(buf_m+len_m, "sst id: %s, lifetime: %d \n", zfile->GetLinkFiles()[0].c_str(), zfile->GetWriteLifeTimeHint());
...
*/
  ...
  for (ZoneExtent* ext : new_extent_list) {
    ...
    len_mi += sprintf(buf_mi+len_mi, "ext_wr: %lx, lifetime: %d, sst name: %s, len: %ld ext_start: %ld \n", ext->zone_->start_, zfile->GetWriteLifeTimeHint(), zfile->GetLinkFiles()[0].c_str(), ext->length_, ext->start_);
	...
	char buf[1000000];
	int len = 0;
	this->mig_+=ext->length_;
    len += sprintf(buf+len, "migration: %ld bytes \n", this->mig_);
  
    std::ofstream f2("test3.txt");
    f2 << buf;
    f2.close();
    ...
    if (zfile->IsSparse()) {
      ...
      len3 += sprintf(buf3+len3, "mig %ld %ld %d %ld \n", ext->zone_->start_, ext->length_, zfile->GetWriteLifeTimeHint(), target_zone->start_);
      ...
      len4 += sprintf(buf4+len4, "mig %ld %ld %d %ld \n", ext->zone_->start_, ext->length_, zfile->GetWriteLifeTimeHint(), target_zone->start_);
  
      std::ofstream f4("test2.txt", std::ios::app);
      f4 << buf4;
      f4.close();
