...
class ZonedBlockDevice {
 ...
 public:
  ...
  std::vector<uint64_t> GetUsedCapacityL2() {
    std::vector<Zone *> Z = GetL2Zone();
    uint64_t usedcap = 0;
    uint64_t freecap = 0;
    for(auto z : Z) {
      usedcap += z->used_capacity_; 
	  if (z->IsFull()) usedcap += (z->max_capacity_ - z->used_capacity_);
      freecap += z->capacity_;
    }
    std::vector<uint64_t> ret;
    ret.emplace_back(usedcap);
    ret.emplace_back(freecap);
    return ret;
  }
  /*modi*/
