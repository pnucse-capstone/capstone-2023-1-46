...
Status ZoneFile::MergeUpdate(std::shared_ptr<ZoneFile> update, bool replace) {
  ...
  fprintf(stdout, "mergeupdate before name: %s\n", linkfiles_[0].c_str());
  linkfiles_.clear();
  for (const auto& name : update->GetLinkFiles()) { fprintf(stdout, "after name: %s\n", name.c_str()); linkfiles_.push_back(name); }
...
/*modi*/
/*
inline char buf_te[10000];
inline int len_te;

inline void buf_out() {
  std::ofstream fout;
  if (!fout.is_open()) { fout.open("test.txt"); }
  if (fout.is_open()) {
    fout << buf_te;
    buf_te[0] = '\0';
    len_te = 0;
...
void ZoneFile::ClearExtents() {
  for (auto e = std::begin(extents_); e != std::end(extents_); ++e) {
    ...
    /*modi*/
    //er_zfile.insert(std::pair<uint64_t, int>(zone->start_, this->GetWriteLifeTimeHint()));

    char buf_te[1000000];
    int len_te = 0;

    len_te += sprintf(buf_te + len_te, "file id: %ld, wr: %lx, lifetime: %d, ext len: %ld\n", this->GetID(), zone->start_, this->GetWriteLifeTimeHint(), (*e)->length_);
    ...
    len += sprintf(buf + len, "era %ld %ld %d\n", zone->start_, (*e)->length_, this->GetWriteLifeTimeHint());
    std::ofstream f2("test2.txt", std::ios::app);
    f2 << buf;
    f2.close();
    /*modi*/
    /*
    if (len_te < 999900) {
	len_te += sprintf(buf_te + len_te, "wr: %lx, lifetime: %d\n", zone->start_, this->GetWriteLifeTimeHint());
    }
    else {
	std::ofstream f("test.txt", std::ios::app);
	f << buf_te;
	...
	buf_te[0] = '\0';
	len_te = 0;
	len_te += sprintf(buf_te + len_te, "wr: %ld, lifetime: %d\n", zone->start_, this->GetWriteLifeTimeHint());
    }
    */
    /*modi*/
...
//    fprintf(stdout, "CloseActiveZone()\n");
...
//  fprintf(stdout, "BufferedAppend\n");
  ...
  while (left) {
    ...
    len_m += sprintf(buf_m+len_m, "Buf %ld %ld %d \n", active_zone_->start_, extent_length, lifetime_);
...
//  fprintf(stdout, "SparseAppend\n");
  if (active_zone_ == NULL) {
//    fprintf(stdout, "AllocNewZone \n");
    s = AllocateNewZone();
    if (!s.ok()) return s;
  }

//  for (auto i : this->GetLinkFiles()) {
//    fprintf(stdout, "fname: %s, ", i.c_str());
//  }
//  fprintf(stdout, "\n");
  while (left) {
    ...
    len_m += sprintf(buf_m+len_m, "Spa %ld %ld %d\n", active_zone_->start_, extent_length, lifetime_);
...
//  fprintf(stdout, "Append\n");
  fprintf(stdout, "append fname: %s\n", this->GetLinkFiles()[0].c_str());
...
//  fprintf(stdout, "~ZonedWritableFile\n");
...
//  fprintf(stdout, "Fsync\n");
  s = DataSync();
//  fprintf(stdout, "DataSync end\n");
  if (!s.ok()) return s;

  /* As we've already synced the metadata in DataSync, no need to do it again */
  if (buffered && !zoneFile_->IsSparse()) { /* fprintf(stdout, "Fsync end\n"); */ return IOStatus::OK(); }

//  fprintf(stdout, "PersistMetadata start Fsync end\n");
...
//    fprintf(stdout, "fname: %s buffersize: %ld\n", zoneFile_->GetLinkFiles()[0].c_str(), buffer_sz);
...
IOStatus ZonedWritableFile::BufferedWrite(const Slice& slice) {
  ...
  //fprintf(stdout, "BufferedWrite\n");
  while (data_left) {
    uint32_t buffer_left = buffer_sz - buffer_pos;
    uint32_t to_buffer;

    if (!buffer_left) {
//      fprintf(stdout, "FlushBuffer start\n");
      s = FlushBuffer();
//      fprintf(stdout, "FlushBuffer end\n");
      if (!s.ok()) return s;
...
IOStatus ZonedWritableFile::PositionedAppend(const Slice& data, uint64_t offset,
                                             const IOOptions& /*options*/,
                                             IODebugContext* /*dbg*/) {
//  fprintf(stdout, "PositionedAppend\n");
  IOStatus s;
  ZenFSMetricsLatencyGuard guard(zoneFile_->GetZBDMetrics(),
                                 zoneFile_->GetIOType() == IOType::kWAL
                                     ? ZENFS_WAL_WRITE_LATENCY
  ...
  if (buffered) {
    buffer_mtx_.lock();
//    fprintf(stdout, "BufferedWrite start\n");
    s = BufferedWrite(data);
    buffer_mtx_.unlock();
  } else {
//    fprintf(stdout, "Append start\n");
    s = zoneFile_->Append((void*)data.data(), data.size());
    if (s.ok()) wp += data.size();
  }
