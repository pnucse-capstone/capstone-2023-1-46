...
IOStatus Zone::Close() {
  ...
  /*modi*/
  //if (lifetime_ == 2 && !IsFull()) { return IOStatus::OK(); }
  /*modi*/
...
//  if (file_lifetime == Env::WLTH_SHORT && zone_lifetime == Env::WLTH_MEDIUM) return 0;
...
IOStatus ZonedBlockDevice::ResetUnusedIOZones() {
  for (const auto z : io_zones) {
    //if (z->lifetime_ == 2 && !(z->IsFull())) { continue; }
    //uint64_t c = 100 - 100 * z->capacity_/z->max_capacity_;
    //if (z->lifetime_ == 3 && !(z->IsFull())) { continue; }
    if (z->Acquire()) {
	...
	len_z += sprintf(buf_z + len_z, "reset zone start: %lx, zone lifetime: %d\n", z->start_, z->lifetime_);
    ...
    len3 += sprintf(buf3+len3, "res %ld %d \n", z->start_, z->lifetime_);
...
IOStatus ZonedBlockDevice::GetBestOpenZoneMatch(
  ...
  for (const auto z : io_zones) {
    if (z->Acquire()) {
			/*modi*/
      if ((/*(z->lifetime_ == 2) || (z->lifetime_ == 3) ||*/ z->used_capacity_ > 0) && !z->IsFull() &&
...
IOStatus ZonedBlockDevice::TakeMigrateZone(Zone **out_zone,
  ...
  if (s.ok() && (*out_zone) != nullptr) {
    ...
    len_m += sprintf(buf_m+len_m, "\nfail migrate s.ok(): %d, *outzone == null: %d\n", s.ok(), (*out_zone) == nullptr);
...
//  fprintf(stdout, "best diff1: %d\n", best_diff);

  // Holding allocated_zone if != nullptr

  if (best_diff >= LIFETIME_DIFF_COULD_BE_WORSE) {
//    fprintf(stdout, "best diff2: %d\n", best_diff);
    bool got_token = GetActiveIOZoneTokenIfAvailable();
