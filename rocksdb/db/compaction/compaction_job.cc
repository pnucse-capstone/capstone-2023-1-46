...
void CompactionJob::Prepare() {
  ...
  if (boundaries_.size() > 1) {
    ...
    /*modi*/
//    fprintf(stdout, "Prepare 11111111\n");
  ...
  if (preserve_time_duration > 0) { // 통과 x
...
//        fprintf(stdout, "pass\n");
  ...
  /*modi*/
    //fprintf(stdout, "files_output.size(): %ld\n", files_output.size());
...
//        fprintf(stdout, "pass\n");
        ...
        if (s.ok() && paranoid_file_checks_) {
          fprintf(stdout, "not pass 15\n");
          OutputValidator validator(cfd->internal_comparator(),
                                    /*_enable_order_check=*/true,
                                    /*_enable_hash=*/true);
          for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
            fprintf(stdout, "not pass 16\n");
          ...
          if (s.ok()) {
            fprintf(stdout, "not pass 17\n");
            s = iter->status();
          }
          if (s.ok() &&
              !validator.CompareValidator(files_output[file_idx]->validator)) {
            fprintf(stdout, "not pass 18\n");
...
void CompactionJob::NotifyOnSubcompactionBegin(
  ...
  /*modi*/
  //fprintf(stdout, "notifyonsubcompactionbegin\n");
  ...
  /*modi*/
//    fprintf(stdout, "onsubcompactionbegin start\n");
...
void CompactionJob::NotifyOnSubcompactionCompleted(
  ...
  /*modi*/
//  fprintf(stdout, "notifyonsubcompactioncompleted\n");
  SubcompactionJobInfo info{};
  sub_compact->BuildSubcompactionJobInfo(info);
  info.job_id = static_cast<int>(job_id_);
  info.thread_id = env_->GetThreadID();
...
void CompactionJob::ProcessKeyValueCompaction(SubcompactionState* sub_compact) {
  ...
  /*modi*/
//  fprintf(stdout, "processkeyvaluecompaction\n");
...
Status CompactionJob::InstallCompactionResults(
  ...
  /*modi*/
//    fprintf(stdout, "InstallCompactionResult\n");
...
void CompactionJob::CleanupCompaction() {
  /*modi*/
  //fprintf(stdout, "CleanupCompaction\n");
