...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  if (immutable_db_options_.enable_pipelined_write) {
    return PipelinedWriteImpl(write_options, my_batch, callback, log_used, // exexex
...
/*modi*/
/*
int prev0, prev1, prev2, prev3, prev4, prev5, prev6;
*/
/*modi*/
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  if (w.state == WriteThread::STATE_MEMTABLE_WRITER_LEADER) {
    ...
    if (memtable_write_group.size > 1 &&
      ...
      /*modi*/
      /* 
      ColumnFamilyData* cfd = GetVersionSet()->GetColumnFamilySet()->GetDefault();
      int levels = cfd->NumberLevels();
      int cnumm[7] = { 0, 0, 0, 0, 0, 0, 0 };
      for(int i = 0; i < levels; i++) {
	int cnt = 0;
	cnt += cfd->current()->storage_info()->LevelFiles(i).size();
	cnumm[i] = cnt;
      }

      bool ischanged = (prev0 != cnumm[0] || prev1 != cnumm[1] || prev2 != cnumm[2] || prev3 != cnumm[3] || 
		        prev4 != cnumm[4] || prev5 != cnumm[5] || prev6 != cnumm[6]);

//      std::ofstream fout_le;
//      fout_le.open("test.txt");
      if (ischanged) {
//	char buf_le[4000];
//	int len_le = 0;
        for(int i = 0; i < levels; i++) {
          auto files = cfd->current()->storage_info()->LevelFiles(i);
  	  if(files.empty()) { break; }
          fprintf(stdout, "level%d: ", i);
//	  len_le += sprintf(buf_le + len_le, "level%d: ", i);

          for(auto f : files) {
            fprintf(stdout, "%lu / ", f->fd.packed_number_and_path_id);
//	    len_le += sprintf(buf_le + len_le, "%lu / ", f->fd.packed_number_and_path_id);
          }
          fprintf(stdout, "\n");
//	  len_le += sprintf(buf_le + len_le, "\n");

        }
	prev0 = cnumm[0]; prev1 = cnumm[1]; prev2 = cnumm[2]; prev3 = cnumm[3]; 
	prev4 = cnumm[4]; prev5 = cnumm[5]; prev6 = cnumm[6]; 
	fprintf(stdout, "\n");
	//len_le += sprintf(buf_le + len_le, "\n");
