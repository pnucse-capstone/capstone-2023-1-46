...
Status DBImpl::CompactRangeInternal(const CompactRangeOptions& options,
  ...
  /*modi*/
  fprintf(stdout, "compactrangeinternal\n");
  auto cfh = static_cast_with_check<ColumnFamilyHandleImpl>(column_family);
  auto cfd = cfh->cfd();
...
Status DBImpl::CompactFilesImpl(
  ...
  fprintf(stdout, "CompactFilesImpl \n");
...
Status DBImpl::ContinueBackgroundWork() {
  /*modi*/
  fprintf(stdout, "continuebackgroundwork\n");
  InstrumentedMutexLock guard_lock(&mutex_);
...
void DBImpl::NotifyOnCompactionBegin(ColumnFamilyData* cfd, Compaction* c,
                                     ...
                                     int job_id) {
  /*modi*/
  //fprintf(stdout, "notifyoncompactionbegin \n");
...
void DBImpl::NotifyOnCompactionCompleted(
  ...
  /*modi*/
  //fprintf(stdout, "notifyoncompactioncompleted \n");
  if (immutable_db_options_.listeners.size() == 0U) {
    return;
  }
  mutex_.AssertHeld();
...
Status DBImpl::RunManualCompaction(
  ...
  /*modi*/
  fprintf(stdout, "runmanualcompaction\n");
...
//    fprintf(stdout, "%d %d %d %d\n", bg_compaction_scheduled_, bg_bottom_compaction_scheduled_, bg_job_limits.max_compactions, unscheduled_compactions_);
...
//    fprintf(stdout, "pass\n");
    if (shutting_down_.load(std::memory_order_acquire)) {
      fprintf(stdout, "not pass2 \n");
      status = Status::ShutdownInProgress();
    } else if (is_manual &&
               manual_compaction->canceled.load(std::memory_order_acquire)) {
      fprintf(stdout, "not pass3 \n");
      status = Status::Incomplete(Status::SubCode::kManualCompactionPaused);
    }
  } else {
    fprintf(stdout, "not pass4 \n");
    status = error_handler_.GetBGError();
  ...
  bool sfm_reserved_compact_space = false;
  if (is_manual) {
    fprintf(stdout, "not pass8 \n");
    ManualCompactionState* m = manual_compaction;
    assert(m->in_progress);
    if (!c) {
      fprintf(stdout, "not pass9 \n");
      m->done = true;
...
//            fprintf(stdout, "pass\n"); 간혹 pass
  ...
  IOStatus io_s;
  if (!c) {
    fprintf(stdout, "not pass21 \n");
    // Nothing to do
    ROCKS_LOG_BUFFER(log_buffer, "Compaction nothing to do");
  } else if (c->deletion_compaction()) {
    fprintf(stdout, "not pass22 \n");
    ...
    *made_progress = true;
    TEST_SYNC_POINT_CALLBACK("DBImpl::BackgroundCompaction:AfterCompaction",
                             c->column_family_data());
  } else if (!trivial_move_disallowed && c->IsTrivialMove()) { // 다음 레벨과 키 범위가 안겹칠때 컴펙션하지않고 SST file을 다음레벨로 그대로 넘김
//    fprintf(stdout, "pass\n");
...
//    fprintf(stdout, "Notify 1 start\n");
    ...
    if (c->compaction_reason() == CompactionReason::kLevelMaxLevelSize &&
      ...
      fprintf(stdout, "not pass24 \n");
      int start_level = c->start_level();
      if (start_level > 0) {
        fprintf(stdout, "not pass25 \n");
...
//    fprintf(stdout, "Notify 2 start\n");
    ...
    // Should handle error?
    /*modi*/
//    fprintf(stdout, "Run() start\n");
    compaction_job.Run().PermitUncheckedError();
    TEST_SYNC_POINT("DBImpl::BackgroundCompaction:NonTrivial:AfterRun");
    mutex_.Lock();

  /*modi*/
//    fprintf(stdout, "BackgroundCompaction \n");
...
//    fprintf(stdout, "Notify 1 end \n");
  ...
  if (is_manual) {
    ...
    if (m->manual_end == nullptr) {
      fprintf(stdout, "not pass35 \n");
      m->done = true;
    }
    if (!m->done) {
      fprintf(stdout, "not pass36 \n");
